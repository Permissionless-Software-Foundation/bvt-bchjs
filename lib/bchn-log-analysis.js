...
class AnalyizeLogs {
  ...
  getSummaryTitle (variant = '') {
    if (!variant) return 'bch.fullstack.cash Analytics:'
    if (variant === 'x402') return 'x402-bch.fullstack.cash Analytics [x402-bch]:'
    return `bch.fullstack.cash Analytics [${variant}]:`
  }
  ...
  getTimespanDays (firstTimestamp, lastTimestamp) {
    const firstMs = new Date(firstTimestamp).getTime()
    const lastMs = new Date(lastTimestamp).getTime()

    if (Number.isNaN(firstMs) || Number.isNaN(lastMs) || lastMs < firstMs) return 'n/a'

    const spanDays = (lastMs - firstMs) / (1000 * 60 * 60 * 24)
    return spanDays.toFixed(2)
  }
  ...
  async publishInsightsSummary (insights, variant = '') {
    ...
    const timespanDays = this.getTimespanDays(
      insights.availability.firstTimestamp,
      insights.availability.lastTimestamp
    )
    ...
    const summaryLines = [
      ...
      `OldestLogTimestamp=${insights.availability.firstTimestamp || 'n/a'}, NewestLogTimestamp=${insights.availability.lastTimestamp || 'n/a'}, TimespanDays=${timespanDays}`,
      'Top5Endpoints:',
      endpointLines,
      'Top5ClientIps:',
      ipLines,
