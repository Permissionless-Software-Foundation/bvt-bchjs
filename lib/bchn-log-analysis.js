...
class AnalyizeLogs {
  ...
  async downloadLogs() {
    ...
    shell.cd(`${__dirname}/../../bchn-logs/`);

    // Exit if the free-bchn-logss dir is not available.
    try {
      const info = shell.exec(`pwd`);
      // console.log(`info: ${info}`)
      if (info.indexOf(`bchn-logs`) === -1)
        throw new Error(`bchn-logs directory is not available.`);
    } catch (err) {
      return false;
    }
  ...
  async usageReport() {
    ...
    shell.exec(
      `echo "High Level Summary" > ${__dirname}/../html/logs/fullstack-analytics/bchn-analytics.txt`
    );
    shell.exec(
      `./bin/run usage-report >> ${__dirname}/../html/logs/fullstack-analytics/bchn-analytics.txt`
    );

    // Post the high-level summary to Slack.
    const data = await this.readFile(
      `${__dirname}/../html/logs/fullstack-analytics/bchn-analytics.txt`
    );
