...
class AnalyizeLogs {
  ...
  async downloadLogs() {
    ...
    shell.cd(`${__dirname}/../../free-bchn-logs/`);

    // Exit if the free-bchn-logss dir is not available.
    try {
      const info = shell.exec(`pwd`);
      // console.log(`info: ${info}`)
      if (info.indexOf(`free-bchn-logs`) === -1)
        throw new Error(`free-bchn-logs directory is not available.`);
    } catch (err) {
      return false;
    }
  ...
  async usageReport() {
    ...
    shell.exec(
      `echo "High Level Summary" > ${__dirname}/../html/logs/fullstack-analytics/free-bchn-analytics.txt`
    );
    shell.exec(
      `./bin/run usage-report >> ${__dirname}/../html/logs/fullstack-analytics/free-bchn-analytics.txt`
    );

    // Post the high-level summary to Slack.
    const data = await this.readFile(
      `${__dirname}/../html/logs/fullstack-analytics/free-bchn-analytics.txt`
    );

    await utils.logAll(`Free BCHN Analytics:`)
  ...
  async endpointReport() {
    ...
    shell.exec(
      `echo " " >> ${__dirname}/../html/logs/fullstack-analytics/free-bchn-analytics.txt`
    );
    shell.exec(
      `echo "Endpoint Report:" >> ${__dirname}/../html/logs/fullstack-analytics/free-bchn-analytics.txt`
    );
    shell.exec(
      `./bin/run endpoint-report >> ${__dirname}/../html/logs/fullstack-analytics/free-bchn-analytics.txt`
    );
  ...
  async errorReport() {
    ...
    shell.exec(
      `./bin/run error-report >> ${__dirname}/../html/logs/fullstack-analytics/free-bchn-analytics.txt`
    );
  }
}
