...
const SERVERS = [
  { serverUrl: "https://abc.fullstack.cash/v4/", testnet: false },
  { serverUrl: "https://bchn.fullstack.cash/v4/", testnet: false },
  { serverUrl: "https://testnet3.fullstack.cash/v4/", testnet: true }
];
  ...
  async slpdbHealth(bchjs) {
    try {
      ...
      if(slpdbBchBlockHeight !== blockHeight) {
        utils.logAll(`Warning: SLPDB may have fallen behind. Current block height: ${blockHeight}, SLPDB block height: ${slpdbBchBlockHeight}`);
        return
      }

      if(slpdbProcessedBlockHeight !== null && slpdbProcessedBlockHeight !== blockHeight) {
        utils.logAll(`Warning: SLPDB may have fallen behind. Current block height: ${blockHeight}, SLPDB *processed* block height: ${slpdbProcessedBlockHeight}`);
        return
      }

      utils.log(`SLPDB connected to ${bchjs.restURL} is synced to the current block height of ${blockHeight}`);
