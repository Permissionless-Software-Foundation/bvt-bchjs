...
const SERVERS = [
  { serverUrl: 'https://bch.fullstack.cash/v6/', testnet: false }
]
...
class LivenessTests {
  ...
  async runTests () {
    try {
      ...
      for (let i = 0; i < SERVERS.length; i++) {
        ...
        if (isTestnet) { addr = 'bchtest:qqnjfxwqt2a4sfmmlg5xlnp5ywaz3ke3ku9e0kd9fy' }
        ...
        if (isTestnet) { addr = 'slptest:qz35h5mfa8w2pqma2jq06lp7dnv5fxkp2shlcycvd5' }

        await this.livenessPsfSlpIndexer(bchjs)

        utils.log(' ')
      }

      await this.livenessFreeBch()
  ...
  async livenessFullNode (bchjs) {
    try {
      await bchjs.Control.getNetworkInfo()
  ...
  async livenessSlpdb (bchjs, addr) {
    try {
      await bchjs.SLP.Utils.balancesForAddress(addr)
  ...
  // Test that the psf-slp-indexer is responsive.
  async livenessPsfSlpIndexer (bchjs) {
    try {
      await bchjs.PsfSlpIndexer.status()
      // console.log(`details: ${JSON.stringify(details,null,2)}`)

      utils.log(`psf-slp-indexer server connected to ${bchjs.restURL} is OK.`)
    } catch (err) {
      utils.logAll(`psf-slp-indexer server connected to ${bchjs.restURL}is DOWN`)

      console.error('Error in liveness.js/livenessPsfSlpIndexer(): ', err)
    }
  }
  ...
  async livenessBlockbook (bchjs, addr) {
    try {
      await this.bchjs.Blockbook.balance(
  ...
  async livenessFreeBch () {
    try {
      ...
      await slpWallet.getBalance(addr)
      // console.log("result: ", result);

      utils.log('free-bch.fullstack.cash is OK.')
    } catch (err) {
      utils.logAll(`Error in livenessFreeBch(): ${err.message}`)
      console.log(err)
    }
  }
}

export default LivenessTests
