...
class PsfToken {
  ...
  async runTests () {
    try {
      ...
      if (burn.reason === 'no_snapshot_before_cutoff') {
        ...
        console.log(`PSF burned in last 24h: ${burn.burnedTokensFloor2}`)
      }
  ...
  async getSlpIndexerData () {
    try {
      ...
      console.log(`All PSF tokens ever minted: ${floor2(maxTokensInCirculation)}`)

      let totalBurned = parseInt(data.tokenData.genesisData.totalBurned, 10)
      totalBurned = totalBurned / 10 ** 8
      console.log(`Total burned: ${floor2(totalBurned)}`)

      const tokensInCirculation = maxTokensInCirculation - totalBurned
      console.log(`Tokens in circulation: ${floor2(tokensInCirculation)}`)
