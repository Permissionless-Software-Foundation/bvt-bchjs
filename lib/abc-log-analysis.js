...
class AnalyizeLogs {
  ...
  async downloadLogs() {
    ...
    shell.cd(`${__dirname}/../../abc-logs/`);

    // Exit if the fullstack-logs dir is not available.
    try {
      const info = shell.exec(`pwd`);
      // console.log(`info: ${info}`)
      if (info.indexOf(`abc-logs`) === -1)
        throw new Error(`abc-logs directory is not available.`);
    } catch (err) {
      return false;
    }
  ...
  async usageReport() {
    ...
    shell.exec(
      `echo "High Level Summary" > ${__dirname}/../html/logs/fullstack-analytics/abc-analytics.txt`
    );
    shell.exec(
      `./bin/run usage-report >> ${__dirname}/../html/logs/fullstack-analytics/abc-analytics.txt`
    );

    // Post the high-level summary to Slack.
    const data = await this.readFile(
      `${__dirname}/../html/logs/fullstack-analytics/abc-analytics.txt`
    );
  ...
  async endpointReport() {
    ...
    shell.exec(
      `echo " " >> ${__dirname}/../html/logs/fullstack-analytics/abc-analytics.txt`
    );
    shell.exec(
      `echo "Endpoint Report:" >> ${__dirname}/../html/logs/fullstack-analytics/abc-analytics.txt`
    );
    shell.exec(
      `./bin/run endpoint-report >> ${__dirname}/../html/logs/fullstack-analytics/abc-analytics.txt`
    );
  ...
  async errorReport() {
    ...
    shell.exec(
      `./bin/run error-report >> ${__dirname}/../html/logs/fullstack-analytics/abc-analytics.txt`
    );
  }
}
