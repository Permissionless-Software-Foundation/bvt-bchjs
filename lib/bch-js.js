...
class Tests {
  ...
  async runTests() {
    ...
    await this.runFreeIntegrationTests();

    await this.runFreeBchnIntegrationTests();
  ...
  async runFreeIntegrationTests() {
    return new Promise((resolve, reject) => {
      try {
        ...
        // const execStr = `BCHJSTOKEN=${config.apiToken} npm run test:integration > ${__dirname}/../html/logs/bitbox/integration-test.txt`;
        const execStr = `npm run test:integration:free > ${__dirname}/../html/logs/bitbox/free-test.txt`;

        shell.exec(execStr, async function(code, stdout, stderr) {
          if (code !== 0) {
            utils.log(`bch-js ABC free-tier integration tests: FAILED`);

            await telegram.send(
              `bch-js ABC free-tier tests failed. Details here: ${
                config.bvtServer
              }/bitbox/free-tests.html`
            );

            // All integration tests succeeded.
          } else {
            utils.log(`bch-js ABC free-tier integration tests: OK`);
          }

          resolve();
        });
      } catch (err) {
        console.log(`Error in bch-js.js/runFreeIntegrationTests(): `, err);
        reject(err);
      }
    });
  }

  // Integration Free BCHN Tests
  async runFreeBchnIntegrationTests() {
    return new Promise((resolve, reject) => {
      try {
        shell.cd(`${__dirname}`); // Ensure the function is on a known directory path
        shell.cd(`${__dirname}/../uut/bch-js`);
        ...
        // const execStr = `BCHJSTOKEN=${config.apiToken} npm run test:integration > ${__dirname}/../html/logs/bitbox/integration-test.txt`;
        const execStr = `npm run test:integration:free:bchn > ${__dirname}/../html/logs/bitbox/free-bchn-test.txt`;

        shell.exec(execStr, async function(code, stdout, stderr) {
          if (code !== 0) {
            utils.log(`bch-js BCHN free-tier integration tests: FAILED`);

            await telegram.send(
              `bch-js BCHN free-tier tests failed. Details here: ${
                config.bvtServer
              }/bitbox/free-tests.html`
            );
            ...
            utils.log(`bch-js BCHN free-tier integration tests: OK`);
          }

          resolve();
        });
      } catch (err) {
        console.log(`Error in bch-js.js/runFreeBchnIntegrationTests(): `, err);
        reject(err);
      }
    });
  }
  ...
  async runTestnetIntegrationTests() {
    return new Promise((resolve, reject) => {
      try {
        ...
        const execStr = `BCHJSTOKEN=${process.env.BCHJSTOKEN} RESTURL=https://tapi.fullstack.cash/v3/ npm run test:integration:testnet > ${__dirname}/../html/logs/bitbox/testnet-test.txt`;
