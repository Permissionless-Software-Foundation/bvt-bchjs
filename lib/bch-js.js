...
class Tests {
  ...
  installBchjs () {
    try {
      ...
      shell.cd('bch-js')
      shell.exec(`npm install > ${__dirname.toString()}/../html/logs/bch-js/install.txt`)
    } catch (err) {
  ...
  runUnitTests () {
    return new Promise((resolve, reject) => {
      try {
        shell.cd(`${__dirname.toString()}`) // Ensure the function is on a known directory path
        shell.cd(`${__dirname.toString()}/../uut/bch-js`)

        const execStr = `npm run test > ${__dirname.toString()}/../html/logs/bch-js/unit-test.txt`
        shell.exec(execStr, async function (code, stdout, stderr) {
          ...
          if (code !== 0) {
            ...
            await telegram.send(
              `bch-js unit test failed. Details here: ${config.bvtServer}/bitbox/unit-tests.html`
            )
  ...
  async runBchnIntegrationTests () {
    return new Promise((resolve, reject) => {
      try {
        ...
        // const execStr = `BCHJSTOKEN=${config.apiToken} npm run test:integration > ${__dirname.toString()}/../html/logs/bitbox/integration-test.txt`;
        const execStr = `BCHJSTOKEN=${config.apiToken} npm run test:integration:bchn > ${__dirname.toString()}/../html/logs/bch-js/bchn-integration-test.txt`
        ...
        shell.exec(execStr, async function (code, stdout, stderr) {
          if (code !== 0) {
            ...
            await telegram.send(
              `bch-js BCHN integration test failed. Details here: ${config.bvtServer}/bch-js/bchn-integration-tests.html`
            )
  ...
  async runAbcIntegrationTests () {
    return new Promise((resolve, reject) => {
      try {
        ...
        // const execStr = `BCHJSTOKEN=${config.apiToken} npm run test:integration > ${__dirname.toString()}/../html/logs/bitbox/integration-test.txt`;
        const execStr = `npm run test:integration:abc > ${__dirname.toString()}/../html/logs/bch-js/abc-integration-test.txt`
        ...
        shell.exec(execStr, async function (code, stdout, stderr) {
          if (code !== 0) {
            ...
            await telegram.send(
              `bch-js ABC tests failed. Details here: ${config.bvtServer}/bch-js/abc-integration-tests.html`
            )
  ...
  async runTestnetIntegrationTests () {
    return new Promise((resolve, reject) => {
      try {
        ...
        const execStr = `BCHJSTOKEN=${process.env.BCHJSTOKEN} RESTURL=https://testnet3.fullstack.cash/v5/ npm run test:integration:testnet > ${__dirname.toString()}/../html/logs/bch-js/testnet3-integration-test.txt`
        ...
        shell.exec(execStr, async function (code, stdout, stderr) {
          if (code !== 0) {
            ...
            await telegram.send(
              `bch-js testnet tests failed. Details here: ${config.bvtServer}/bch-js/testnet3-integration-tests.html`
            )
